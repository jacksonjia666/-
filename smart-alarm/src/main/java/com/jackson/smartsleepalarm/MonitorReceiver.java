...
public final class MonitorReceiver extends BroadcastReceiver {
    ...
    private void scheduleFromSleep(Context c,long off,long now){
        Calendar deadline=Calendar.getInstance();deadline.setTimeInMillis(now);deadline.set(Calendar.HOUR_OF_DAY,8);deadline.set(Calendar.MINUTE,15);deadline.set(Calendar.SECOND,0);deadline.set(Calendar.MILLISECOND,0);
        if(deadline.getTimeInMillis()<=now)deadline.add(Calendar.DAY_OF_YEAR,1);
        long wake=Math.min(off+8*60*60_000L+30*60_000L,deadline.getTimeInMillis());
